...
class Learner():
    def __init__(self,
        ...
        # model create
        self.actval_ext_model = actval_func_model(self.model_builder, optim.Adam, uvfa_ext)
        self.actval_ext_model_target = actval_func_model(self.model_builder, optim.Adam, uvfa_ext)
        self.actval_ext_model_target.load_state_dict(self.actval_ext_model.state_dict())
        if self.enable_intrinsic_actval_model:
            self.actval_int_model = actval_func_model(self.model_builder, optim.Adam, uvfa_int)
            self.actval_int_model_target = actval_func_model(self.model_builder, optim.Adam, uvfa_int)
            self.actval_int_model_target.load_state_dict(self.actval_int_model.state_dict())
        if self.enable_intrinsic_reward:
            self.rnd_target_model = rnd_model(self.model_builder, None)
            self.rnd_train_model = rnd_model(self.model_builder, optim.Adam)
            self.emb_model = embedding_model(self.model_builder)
            self.emb_classifier_model = embedding_model_classifier(self.model_builder, optim.Adam)

            # self.mbedding_model(self.model_builder, self.emb_train_model, self.emb_model)
    ...
    def train(self):
        ...
        # RemoteMemory Do not learn until a certain number of are accumulated.
        ...
        # memory Get status based on priority from
        indexes = []
        batchs = []
        weights = []
        memory_types = []
    ...
    def train_model(self, indexes, batchs, weights, memory_types):
        ...
        # Output current Q network for update
        state0_qvals = self.actval_ext_model(torch.cat(state0_batch_ext, dtype=torch.float), self.batch_size)
        ...
        if self.enable_intrinsic_actval_model:
            ...
            # qvals
            state0_qvals_int = self.actval_int_model(torch.cat(state0_batch_int, dtype=torch.float), self.batch_size)
        

        if self.enable_intrinsic_reward:
            # emb network
            emb_act_batch = np.asarray(emb_act_batch)
            self.emb_classifier_model.train_on_batch(torch.tensor(state0_batch, dtype=torch.float), torch.tensor(state1_batch, dtype=torch.float), emb_act_batch)

            # rnd network
            state1_batch = np.asarray(state1_batch)
            rnd_target_val = self.rnd_target_model(torch.tensor(state1_batch), self.batch_size)
            self.rnd_train_model.train_on_batch(torch.tensor(state1_batch), rnd_target_val)


        if self.enable_double_dqn:
            # TargetNetworkとQNetworkのQ値を出す
            state1_qvals_model = self.actval_ext_model(torch.tensor(state1_batch_ext, dtype=torch.float), self.batch_size)
            state1_qvals_target = self.actval_ext_model_target(torch.tensor(state1_batch_ext, dtype=torch.float), self.batch_size)
            if self.enable_intrinsic_actval_model:
                #state1_qvals_model_int = self.actval_int_model(torch.tensor(state1_batch_int, dtype=torch.float), self.batch_size)
                state1_qvals_target_int = self.actval_int_model_target(torch.tensor(state1_batch_int, dtype=torch.float), self.batch_size)
        else:
        ...
        for i in range(self.batch_size):
            ...
            if self.enable_intrinsic_actval_model:
                # Perhaps the Q value reference for internal rewards should be combined with external rewards
                # if self.enable_double_dqn:
    ...
    #         qvals_ext = self.actval_ext_model(state_batch_ext, self.batch_size)
    ...
    #
    #             # emb network
    #             self.emb_classifier_model.train_on_batch([state0, state1], emb_act_batch)
    #
    #             # rnd network
    ...
    def add_exp(self, exp):
        ...
        if terminal:
            ...
            # sync emb
            if self.enable_intrinsic_reward:
                self.emb_model.fc1.weight = self.emb_classifier_model.fc1.weight
                # self.model_builder.sync_embedding_model(self.emb_classifier_model, self.emb_model)
